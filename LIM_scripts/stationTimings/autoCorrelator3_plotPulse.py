...
from LoLIM.IO.raw_tbb_IO import filePaths_by_stationName, MultiFile_Dal1, read_station_delays
...
def plot_stations_AllData(timeID, input_file_name, source_XYZT, known_station_delays, stations="all", referance_station="CS002", min_antenna_amplitude=10, skip_stations=[], 
                          antennas_to_exclude=[], seperation_factor=1):
    """Plot all pulses on all stations in an autocorrelator file. polarization should be 0 or 1. stations should be "all", "RS", or "CS" """
    ...
    for (sname, stat_group) in data_file.items():
        ...
        trace_max = 0.0
        for ant_name, ant_dataset in stat_group.items():
            trace = np.array( ant_dataset[ 1 ])
            if np.max(trace)>trace_max:
                trace_max = np.max(trace)
            trace = np.array( ant_dataset[ 3 ])
            if np.max(trace)>trace_max:
                trace_max = np.max(trace)
        ...
        for ant_name, ant_dataset in stat_group.items():
            ...
            even_peak_time = ant_dataset.attrs[ 'PolE_timeOffset' ] - total_correction
            odd_peak_time = ant_dataset.attrs[ 'PolO_timeOffset' ] - total_correction
            
            trace_time = np.arange(len(trace))*5.0E-9 + ant_dataset.attrs['starting_index']*5.0E-9
            even_trace_time = trace_time + even_peak_time
            odd_trace_time = trace_time + odd_peak_time
            
            plt.plot(even_trace_time, even_trace/trace_max + step, color = 'g')
            plt.plot(even_trace_time, even_HE/trace_max + step, color = 'g')
            
            plt.plot(odd_trace_time, odd_trace/trace_max + step, color = 'm')
            plt.plot(odd_trace_time, odd_HE/trace_max + step, color = 'm')
