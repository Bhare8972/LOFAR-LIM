...
#        self.histogram_axes   = self.fig.add_subplot(self.lower_gs[0,2])
        ...
        self.AltVsT_axes.set_xlabel("time (seconds from arbitrary zero)", fontsize=self.axis_label_size)
    ...
    def replot_data(self):
        ...
        self.PolE_AltVsT_paths = self.AltVsT_axes.scatter(x=self.PolE_masked_T_data, y=self.PolE_masked_alt_data, c=self.PolE_masked_color_data, marker='s', s=self.marker_size, cmap=self.cmap)
        self.PolO_AltVsT_paths = self.AltVsT_axes.scatter(x=self.PolO_masked_T_data, y=self.PolO_masked_alt_data, c=self.PolO_masked_color_data, marker='D', s=self.marker_size, cmap=self.cmap)
