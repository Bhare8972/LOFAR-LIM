...
from LoLIM.IO.calFiles import total_cal_object, read_cal_file,  read_antenna_delays, read_bad_antennas, read_antenna_pol_flips, read_station_delays
...
class TBBData_Dal1:
    ...
    def __init__(self, filename, force_metadata_ant_pos=False, forcemetadata_delays=True, metadata_adjusts={}):
        ...
        if 'TELESCOPE_VERSION' in self.file.attrs:
            if self.file.attrs['TELESCOPE_VERSION'] == '2.0':
                print('ERROR: am opening L2 data with L1 data reader!')
                quit()
...
class LOFAR1_tbb_reader:
    ...
    def __init__(self, filename_list, force_metadata_ant_pos=True, total_cal=None,
        ...
        if total_cal is not None:
            ...
            if polarization_flips is not None and pol_flips_are_bad:
                for even_ant in polarization_flips:
                    ...
                    bad_antennas.append( even_antName_to_odd( even_ant ) )
        ...
        for TBB_file in self.files:
            ...
            for ant_i,ant_name in enumerate(file_ant_names):
                ...
                if antName_is_even(ant_name):
                    ...
                    even_ant_name =  odd_antName_to_even( ant_name )
        ...
        self.unusedAnt_locations = {} ## key is external index, value is external index of antenna who has same location
        if not only_complete_pairs:
            for ant_name, to_file in zip(unused_antenna_names, unused_antenna_to_file):
                if antName_is_even(ant_name): ##check if antenna is even
                    
                    self.dipoleNames.append( ant_name )
                    self.antenna_to_file.append( to_file )

                    self.unusedAnt_locations[ len(self.dipoleNames) ] = len(self.dipoleNames)-1
                    
                    self.dipoleNames.append( even_antName_to_odd( ant_name ) ) ## add the odd antenna
                    # self.dipoleNames.append( None ) ## add the odd antenna
                    self.antenna_to_file.append( None ) ## doesn't exist in a file
                    
                else:
                    
                    self.dipoleNames.append( odd_antName_to_even( ant_name ) ) ## add the even antenna
                    # self.dipoleNames.append( None) ## add the even antenna
                    self.antenna_to_file.append( None ) ## doesn't exist in a file


                    self.unusedAnt_locations[ len(self.dipoleNames)-1 ] = len(self.dipoleNames)
...
# 7 X    outer
        ...
        self.geo_delay_temp = None ## used to hold a temporary array to make geometric delay caluclations more efficent
    ...
    def get_antenna_names(self):
        """return name of antenna as a list of strings. For LOFAR 1.0 this is RCU id, and the physical antenna depends on the antennaSet.
        ...
        Garunteed to be 9 digits long. First three is station ID (not name), and last three uniquely identify antenna (given antennaSet and station name).
        In LOFAR2.0 this may not conform to official antenna name."""
        ret = []
    ...
    def has_antenna(self, antenna_name=None, antenna_index=None):
        ...
        if antenna_name is not None:
            if antenna_name in self.dipoleNames:
                ...
                antenna_index = [i for i,a in enumerate(self.name_adjusts) if a==internal_name_index][0]   #self.name_adjusts.index( internal_name_index )
            else:
                return False
    ...
    def get_full_data_lengths(self):
        ...
        for external_index, internal_index in enumerate( self.name_adjusts ):
            ...
            ret[external_index] = self.DataLengths[internal_index]
    ...
    def get_ITRF_antenna_positions(self, out=None):
        ...
        for external_index, internal_index in enumerate(self.index_adjusts):
            ...
            R = self.antenna_to_file[ internal_index ]

            if R is None:
                continue
            else:
                TBB_file,station_ant_i = R
                out[ external_index ] = TBB_file.ITRF_dipole_positions[station_ant_i]

        for external_index, other_external_index in self.unusedAnt_locations.items(): ## fix antenna locations that don't exist
            out[ external_index ] = out[other_external_index]
    ...
    def get_data(self, start_index, num_points, antenna_index=None, antenna_name=None, out=None):
        ...
        try:
            if out is None:
                ...
                DS = TBB_file.file[ TBB_file.stationKey ][ antenna_name ]
                #DS.read_direct(out, np.s_[start_index:start_index+num_points], np.s_[0:num_points])
                DS.read_direct(out, np.s_[initial_point:final_point], np.s_[0:num_points])
                RET = out
