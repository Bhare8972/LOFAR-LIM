...
class TBBData_Dal1:
    ...
    def __init__(self, filename):
        ...
        if self.have_metadata:
            ...
            self.ITRF_dipole_positions = np.empty((len(self.dipoleNames), 3))
            self.calibrationDelays = np.empty( len(self.dipoleNames) )
            ...
            self.ITRF_dipole_positions = md.getItrfAntennaPosition(self.StationName, self.antennaSet)
...
class MultiFile_Dal1:
    ...
    def __init__(self, filename_list):
        self.files = [TBBData_Dal1(fname) for fname in filename_list]
